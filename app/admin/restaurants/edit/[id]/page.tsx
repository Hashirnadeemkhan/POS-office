// src/app/admin/restaurants/edit/[id]/page.tsx
"use client";
...
export default function EditRestaurantPage({ params }: { params: { id: string } }) {
  ...
  const fetchRestaurant = useCallback(async () => {
    ...
    try {
      ...
      const tokenActivationDate = restaurant.tokenActivationDate
        ? new Date(restaurant.tokenActivationDate).toISOString().split("T")[0]
        : "";
      const tokenExpiresAt = restaurant.tokenExpiresAt
        ? new Date(restaurant.tokenExpiresAt).toISOString().split("T")[0]
        : "";

      setOriginalToken(restaurant.activationToken || "");
      setOriginalEmail(restaurant.email || "");
      ...
      setFormData({
        ...
        activationToken: restaurant.activationToken || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`/api/restaurants/update/${params.id}`, {
        method: "PATCH",
      ...
      if (password) {
        const passwordResponse = await fetch("/api/update-user", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            uid: params.id,
