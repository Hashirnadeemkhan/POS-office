...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      if (!restaurantData) {
        console.log("POST /api/impersonate: Restaurant data is undefined:", restaurantId);
        return NextResponse.json({ error: "Restaurant data not found" }, { status: 404 });
      }

      if (!restaurantData.isActive) {
        console.log("POST /api/impersonate: Restaurant is deactivated:", restaurantId);
        return NextResponse.json({ error: "Cannot impersonate a deactivated restaurant" }, { status: 403 });
      }

      // Generate a custom token for the restaurantId
      const customToken = await posAdminAuth.createCustomToken(restaurantId, {
      ...
      const response = NextResponse.json({ sessionToken, restaurantId, customToken }, { status: 200 });
    ...
    console.error("POST /api/impersonate: Error:", error, "Stack:", error instanceof Error ? error.stack : undefined);
